...
class RaccoonConfirmDialog : SimpleDialogFragment() {
  ...
  private fun setupTitle(dialog: Dialog) {
    dialog.findViewById<TextView>(R.id.titleTextView)
        .text = title
  }

  private fun setupMessage(dialog: Dialog) {
    dialog.findViewById<TextView>(R.id.messageTextView)
        .text = message
  }
  ...
  private fun setupButtons(dialog: Dialog) {
    ...
    dialog.findViewById<CheckBox>(R.id.checkbox)
        .setOnCheckedChangeListener { _, b ->
          viewModel.onCheck(b)
        }
    arguments?.let {
      val showTwiceDisplayCheckBox = it.getBoolean(ARG_SHOW_TWICE_DISPLAY_CHECK_BOX, false)
      dialog.findViewById<LinearLayout>(R.id.twiceDisplayLayout)
          .visibility = if (showTwiceDisplayCheckBox) View.VISIBLE else View.GONE
    }
  }
  ...
  companion object {
    ...
    fun createDialog(
      viewModel: RaccoonConfirmViewModel,
      title: String,
      message: String,
      buttonText: String,
      showTwiceDisplayCheckBox: Boolean = false
    ): RaccoonConfirmDialog {
